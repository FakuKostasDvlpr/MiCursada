...
- Supabase (`@supabase/supabase-js` + `@supabase/ssr`) para datos, Auth (usuario sombra) y Storage (avatares). `zod` para validación, `lucide-react` para íconos, `date-fns`/`date-fns-tz` para fechas, `sanitize-html` para el contenido del aula, `vitest` para tests. **Nada más** — si una dependencia no la usa nadie, se va.
...
- **Dos modos de persistencia, un solo código.** Con Supabase configurado (multiusuario), todo vive en Postgres con RLS; sin configurar (desarrollo), la app cae al snapshot + overlays de `datos/`. La decisión la toma `supabaseConfigurado()` en queries y actions — las pantallas no saben en qué modo están.
